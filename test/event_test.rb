...
class EventTest < Minitest::Test
  ...
  def test_it_can_tell_which_trucks_sell_an_item
    event = Event.new("South Pearl Street Farmers Market")
    food_truck1 = FoodTruck.new("Rocky Mountain Pies")
    item1 = Item.new({name: 'Peach Pie (Slice)', price: "$3.75"})
    item2 = Item.new({name: 'Apple Pie (Slice)', price: '$2.50'})
  ...
  def test_it_can_get_total_inventory
    ...
    event.add_food_truck(food_truck2)
    event.add_food_truck(food_truck3)
    expected = {quantity: 100, food_trucks: [food_truck1, food_truck3]}
    assert_equal expected, event.total_inventory[item1]
  end
  ...
  def test_it_can_sort_items
    ...
    event.add_food_truck(food_truck2)
    event.add_food_truck(food_truck3)
    assert_equal ["Apple Pie (Slice)", "Banana Nice Cream", "Peach Pie (Slice)", "Peach-Raspberry Nice Cream"],
